...
from firetail.utils.formatters import bold, make_embed
...
class Context(commands.Context):
    ...
    @property
    def dest(self):
        return self.author if self.bot.config.dm_only else self
    ...
    async def embed(
        ...
        if not embed:
            embed = make_embed(
                title=title, content=description, msg_type=msg_type, title_url=title_url, msg_colour=colour, icon=icon,
                thumbnail=thumbnail, image=image, guild=self.guild, subtitle=subtitle, subtitle_url=subtitle_url
            )
        else:
...
class GetTools:
    ...
    def __call__(self, iterable, **attrs):
        return self.get(iterable, **attrs)
